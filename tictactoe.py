...
class Grid:
    ...
    def __init__(self, rows, cols):
        ...
        if rows != cols:
            raise ValueError("rows and cols must be equal")
    ...
    def winner(self):
        ...
        for x in self._victory_routes:
            if x >= self._connects_to_win:
                return 'O'
            elif x <= -(self._connects_to_win):
                return 'X'
...
class GameInterface:
    ...
    def print_game(self):
        ...
        for rows in xrange(max_row):
            ...
            # print(game_grid._victory_routes)
            #uncomment to see victory progression
